...
export async function GET(request) {
  ...
  const rows = snapshot.providers.map((provider) => {
    ...
    return `<tr>
      ...
      <td>${badge(provider.metricool_verified, "Metricool OK", "Pas de Metricool")}<div class="detail">${provider.chatgpt_pilotable ? "Pilotable depuis ChatGPT aujourd’hui" : "Pas encore pilotable depuis ChatGPT"}</div></td>
      ...
      <td><div class="action-text"><strong>Blocage externe :</strong> ${esc(provider.external_blocker)}</div><div class="action-text"><strong>Session :</strong> ${esc(provider.authorization_session_step)}</div>${portal ? `<a class="portal" href="${esc(portal)}" target="_blank" rel="noopener noreferrer">Portail développeur ↗</a>` : ""}</td>
      <td>${provider.manual_only ? '<span class="missing">Validation externe requise</span>' : oauth}</td>
    </tr>`;
  }).join("");
  ...
  <div class="table-wrap"><table><thead><tr><th>Réseau</th><th>Phase</th><th>Voie actuelle</th><th>Serveur</th><th>OAuth</th><th>Publication</th><th>Analytics</th><th>Callback</th><th>Fallback webhook</th><th>Blocage / prochaine session</th><th>Validation</th></tr></thead><tbody>${rows}</tbody></table></div>
  <p class="foot">Les client secrets et tokens restent dans les variables serveur/Vercel ou dans le coffre OAuth AES-256-GCM. Metricool est une voie opérationnelle de secours pour les réseaux déjà reliés, sans prétendre que l’API directe Hibou est autorisée. X reste sans dépense automatique. Aucun bouton de cette page ne publie un contenu public ; « Valider l’autorisation » ouvre uniquement le consentement OAuth.</p>
  </main></body></html>`;
