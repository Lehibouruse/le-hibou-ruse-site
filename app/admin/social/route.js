...
const SETUP = {
  reddit: {
    ...
    note: "Obtenir d’abord l’accord API commercial Reddit. Ensuite configurer l’app Web OAuth et autoriser uniquement le compte du Hibou ; aucune publication avant choix d’un subreddit compatible et validation humaine.",
  },
  youtube: {
    label: "YouTube",
  ...
  linkedin: {
    ...
    note: "Créer l’app, configurer OAuth et obtenir les produits/permissions adaptés au profil ou à la Page.",
  },
  pinterest: {
    label: "Pinterest",
  ...
  x: {
    ...
    note: "Créer le projet/app OAuth 2.0, activer les droits d’écriture et déclarer le callback. Toute dépense reste soumise à validation humaine explicite.",
  },
  threads: {
    label: "Threads",
    portal: "https://developers.facebook.com/apps/",
    note: "Créer/configurer l’app Threads chez Meta et autoriser threads_basic + threads_content_publish + threads_manage_insights.",
  },
};
...
export async function GET(request) {
  ...
  try { fingerprint = vaultFingerprint(env); } catch {}
  ...
  const html = `<!doctype html><html lang="fr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow"><title>Connexions sociales · Le Hibou Rusé</title><style>
    ...
    <h1>Connexions sociales</h1><p class="sub">OAuth direct · tokens chiffrés · configuration non secrète centralisée dans Airtable.</p>
    ...
    <p class="foot">Ordre conseillé : 1) créer les apps et leurs secrets serveur ; 2) copier exactement les callbacks affichés ; 3) revenir ici et cliquer Connecter ; 4) conserver social_test_mode=TRUE jusqu’aux dry-runs et validations humaines. Les scopes, versions et identifiants non secrets sont lus depuis Airtable ; les secrets restent uniquement côté serveur. Le domaine d4d5d6.com pourra remplacer l’alias Vercel après validation DNS sans bloquer les premiers OAuth. Snapchat reste séparé : la publication organique serveur dépend d’un accès produit/approbation Snap distinct.</p>
  </main></body></html>`;
