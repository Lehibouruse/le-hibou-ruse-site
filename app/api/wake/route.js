...
export async function POST(request) {
  try {
    ...
    const aiEnabled = isProjectAiEnabled(process.env);
    const aiBlocked = circuit.active || !aiEnabled;
    const candidate = aiBlocked ? records.find((job) => !requiresOpenAi(job)) || null : records[0] || null;
    ...
    if (dryRun || !candidate) {
      return NextResponse.json({
        ...
        reason: !candidate && records.some(requiresOpenAi) ? (!aiEnabled ? "openai_api_disabled" : circuit.active ? "openai_credit_circuit_open" : "") : "",
