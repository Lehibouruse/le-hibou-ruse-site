...
export default async function ArticlePage({ params }) {
  ...
  if (!article) notFound();
  return <main className="legal article-page"><Brand /><div className="eyebrow dark"><span /> {article.Catégorie || "Décryptage"}</div><h1>{article.Titre}</h1><p className="article-summary">{article.Résumé}</p><div className="legal-copy article-body">{String(article.Contenu || "").split("\n").filter(Boolean).map((paragraph, index) => <p key={index}>{paragraph}</p>)}</div><p className="article-disclaimer">Information générale — aucune recommandation personnalisée.</p><a className="text-back" href="/">← Retour à l’accueil</a></main>;
}
