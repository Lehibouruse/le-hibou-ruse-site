...
export default function Page() {
  return <LegalPage title="Confidentialité">
    ...
    <p>Vercel Web Analytics est conçu pour produire des statistiques agrégées sans cookie publicitaire tiers et, dans sa configuration standard, sans identifiant personnel persistant permettant de suivre un visiteur entre plusieurs sites. Le site peut en outre conserver des informations d’attribution first-party limitées lorsqu’un visiteur arrive via une campagne du Hibou Rusé.</p>
    <p>Cette attribution peut inclure la source, la campagne, l’identifiant du contenu, la page d’entrée et le référent, associés à un identifiant de session aléatoire. Elle n’a pas pour objet de constituer un profil publicitaire inter-sites, n’utilise pas de fingerprinting et ne doit pas contenir de donnée bancaire sensible.</p>
    <p><strong>Avant lancement, tout stockage ou accès dans le terminal qui ne serait pas strictement nécessaire ou qui ne remplirait pas les conditions d’une exemption devra être conditionné à un consentement préalable conforme.</strong> Aucun traceur marketing tiers optionnel ne doit être activé sans mécanisme de consentement adapté.</p>
