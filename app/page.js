...
export default async function Home() {
  ...
  const earlyAccess = readiness.earlyAccess === true;
    ...
    <>
      ...
      <main id="top">
        ...
        <section className="ebook" id="ebook"><div><div className="eyebrow"><span /> Guide pratique</div><h2>{ebook.Titre || product.Produit || "Le guide du Hibou Rusé"}</h2><p className="ebook-intro">{ebook["Sous-titre"] || ebook.Contenu || product.Description}</p>
          ...
          <div className="price"><span className="price-amount">{config.ebook_price || product["Prix €"] || 29}&nbsp;€</span> <small>paiement unique</small></div>{checkoutUrl ? <><TrackedLink event="checkout_opened" className="button" href={checkoutUrl} target="_blank" rel="noopener noreferrer">{ctaText}</TrackedLink><p className="checkout-note">Paiement sécurisé par Lemon Squeezy, Merchant of Record · accès protégé envoyé par e-mail</p>{earlyAccess && <p className="checkout-note"><strong>Version partielle actuelle :</strong> Vous recevez immédiatement le PDF V1 disponible aujourd’hui. Le guide est encore en cours d’enrichissement.</p>}</> : <><button className="button disabled" disabled>{ctaText} · bientôt disponible</button><p className="checkout-note">Ouverture commerciale après validation des dépendances techniques.</p></>}
