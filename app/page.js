  ...
  return (
    <>
      <header className="header"><nav className="nav" aria-label="Navigation principale">
        <a className="brand" href="#top"><img src="/hibou.svg" alt="" />Le Hibou Rusé</a>
        <div className="navlinks"><a href="#decrypter">Décryptages</a><a href="#methode">D4 → D6</a><a href="#articles">Articles</a><a href="#montage">Montage personnalisé</a></div>
        <a className="nav-cta" href="#ebook">L’e-book</a>
      </nav></header>
      <main id="top">
        ...
        </div><aside className="hero-card" aria-label="La méthode du Hibou"><img src="/hibou.svg" alt="Logo du Hibou Rusé" /><div className="level-line"><strong>D4</strong><span>Optimisation documentée</span></div><div className="level-line"><strong>D5</strong><span>Structuration approfondie</span></div><div className="level-line"><strong>D6</strong><span>Frontière à faire valider</span></div></aside></div></section>
        ...
        <section className="section" id="articles"><div className="section-head"><div><div className="eyebrow dark"><span /> Décryptages récents</div><h2>Comprendre avant<br />de décider.</h2></div></div><div className="articles">{articles.length ? articles.map((article) => <article className="article" key={article.id}><div className="tag">{article.fields.Catégorie || "Décryptage"}</div><h3>{article.fields.Titre}</h3><p>{article.fields.Résumé}</p><a href={`/articles/${article.fields.Slug}`}>Lire le décryptage <span>↗</span></a></article>) : <article className="article empty"><div className="tag">Bientôt</div><h3>Les premiers décryptages arrivent.</h3><p>Ils seront publiés directement depuis le cockpit éditorial.</p></article>}</div></section>
        <section className="ebook" id="ebook"><div><div className="eyebrow"><span /> Le guide</div><h2>{product.Produit || ebook.Titre || "L’e-book du Hibou Rusé"}</h2><p>{product.Description || ebook.Contenu}</p><div className="price">{config.ebook_price || product["Prix €"] || 29} € <small>paiement unique</small></div>{checkoutUrl ? <TrackedLink event="checkout_opened" className="button" href={checkoutUrl} target="_blank" rel="noopener noreferrer">{ctaText}</TrackedLink> : <button className="button disabled" disabled>{ctaText} · bientôt disponible</button>}</div><div className="book-mark"><img src="/hibou.svg" alt="" /><span>LE GUIDE DU</span><strong>HIBOU<br />RUSÉ</strong><small>D4 → D6</small></div></section>
      ...
      <footer className="footer"><div className="footer-inner"><div className="brand footer-brand"><img src="/hibou.svg" alt="" />Le Hibou Rusé</div><p>{footer.Contenu || "Contenus pédagogiques et informatifs. Ils ne constituent pas un conseil individualisé."}</p><div><a href="/mentions-legales">Mentions légales</a><a href="/confidentialite">Confidentialité</a><a href="/conditions-utilisation">Conditions</a></div></div></footer>
      <a className="mobile-cta" href="#ebook">L’e-book · 29 €</a>
    </>
  );
}
