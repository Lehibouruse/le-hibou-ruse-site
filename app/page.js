    ...
    <>
      ...
      <main id="top">
        <section className="hero"><div className="hero-grid"><div>
          ...
          <p>{hero["Sous-titre"] || "Des stratégies optimisées, ingénieuses, parfois agressives. Des cas concrets, des chiffres et les risques explicités."}</p>
        ...
        </div><aside className="hero-card" aria-label="La méthode du Hibou"><img src={logoUrl} alt="Emblème du Hibou Rusé" /><p className="hero-card-title">Trois niveaux de montage. Jusqu’où peut-on pousser l’optimisation ?</p><div className="level-line"><strong>D4</strong><span>Solide et documenté</span></div><div className="level-line"><strong>D5</strong><span>Agressif mais argumentable</span></div><div className="level-line"><strong>D6</strong><span>Limite · selon interprétation</span></div></aside></div></section>
        ...
        <section className="ebook" id="ebook"><div><div className="eyebrow"><span /> Le guide du Hibou Rusé</div><h2>{ebook.Titre || product.Produit || "Le guide du Hibou Rusé — 29 €"}</h2><p>{ebook.Contenu || product.Description}</p><ul className="ebook-benefits"><li>Ramener légalement l’imposition à zéro dans certaines configurations</li><li>Créer un modèle rentable autour d’aides, de subventions et de l’entrepreneuriat social</li><li>Faire sortir légalement de la valeur d’une entreprise au-delà du seul couple salaire / dividendes</li></ul><p className="checkout-note">Pas un guide basique PEA / assurance-vie / 150-0 B ter : ici, on cherche les montages moins évidents.</p><div className="price">{config.ebook_price || product["Prix €"] || 29} € <small>paiement unique</small></div>{checkoutUrl ? <><TrackedLink event="checkout_opened" className="button" href={checkoutUrl} target="_blank" rel="noopener noreferrer">{ctaText}</TrackedLink><p className="checkout-note">Paiement sécurisé chez notre prestataire · accès envoyé par e-mail</p></> : <button className="button disabled" disabled>{ctaText} · bientôt disponible</button>}</div><div className="book-mark" aria-label="Couverture du guide"><img src={logoUrl} alt="" /><span>LE GUIDE DU</span><strong>HIBOU<br />RUSÉ</strong><small>COMPRENDRE · EXPLOITER · ARBITRER</small></div></section>
        ...
        <section className="method" id="methode"><div className="method-inner"><div><div className="eyebrow"><span /> Jusqu’où peut-on optimiser ?</div><h2>{method.Titre || "Trois niveaux de montage. Trois niveaux d’audace."}</h2><p className="method-intro">{method["Sous-titre"]}</p></div><div className="levels"><article><b>D4</b><div><h3>Solide et documenté</h3><p>Montage légal, propre et difficile à contester lorsque les conditions sont réellement remplies.</p></div></article><article><b>D5</b><div><h3>Agressif mais argumentable</h3><p>On pousse les textes, exceptions et interactions plus loin. Le montage reste défendable, mais exige une documentation sérieuse.</p></div></article><article><b>D6</b><div><h3>Limite · selon interprétation</h3><p>Très agressif : le montage peut tenir ou tomber selon les faits, la rédaction et l’interprétation retenue.</p></div></article></div></div></section>
        ...
        <section className="montage" id="montage"><div className="montage-inner"><div><div className="eyebrow dark"><span /> Sur mesure</div><h2>{strategy.Titre || "Bilan personnel & montage personnalisé"}</h2><p>{strategy.Contenu || strategy["Sous-titre"]}</p><div className="notice"><strong>Le cadre est clair.</strong> Contenu pédagogique et informatif, pas de conseil juridique, fiscal ou financier individualisé. Validation professionnelle lorsque le sujet l’exige.</div></div><LeadForm /></div></section>
      </main>
