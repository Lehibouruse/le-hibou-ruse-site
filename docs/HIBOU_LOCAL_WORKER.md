...
Le téléchargement local peut rester **sans token serveur**. Le reporting vers `/api/local-worker-status` est une capacité séparée et fail-closed.

Pour l'activer, `HIBOU_LOCAL_REPORT_TOKEN` doit être configuré **à la fois** côté serveur/Vercel et localement sur le PC worker. Le bootstrap ne crée ni ne demande ce secret automatiquement.
