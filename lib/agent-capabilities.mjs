...
export function workerInstructions(action) {
  ...
  return action === "CREATE_VIDEO"
    ? `${common}\nProduis seulement un brouillon MP4 vertical consultable. Après register_video_draft, termine waiting_for_human pour validation de Marc; ne bloque pas avant.`
    : `${common}\nUPDATE_SITE est ici une mission globale: parameters.key n'est pas requis. Lis le contexte Airtable et le dépôt, puis améliore le site sur une branche dédiée. La fusion automatique n'est autorisée qu'après tests, build, périmètre et contrôle Vercel réussis.`;
}
