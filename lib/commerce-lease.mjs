...
export function refundDeliveryStatus(status) {
  if (status === "revoking") return "revoking";
  if (status === "manual_review" || status === "failed") return "manual_review";
  if (["delivered", "processing", "revocation_pending"].includes(status)) return "revocation_pending";
  return "revoked";
}
