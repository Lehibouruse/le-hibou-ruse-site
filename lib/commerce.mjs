...
function safeDigifyUrl(value, fallback = "") {
  const raw = String(value || fallback || "").trim();
  if (!raw) return "";
  const url = new URL(raw);
  if (url.protocol !== "https:" || url.hostname !== "api.digify.com") {
...
async function digifyPost(request, label) {
  ...
  if (!response.ok) {
    ...
    // 429 is a clear throttling response and can be retried. A timeout/5xx after a
    // state-changing POST can be ambiguous: the external action may already exist.
