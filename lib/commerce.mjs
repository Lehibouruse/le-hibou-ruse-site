...
import { DIGIFY_ADD_RECIPIENT_DEFAULT_URL, DIGIFY_FILE_SEARCH_URL, DIGIFY_RECIPIENT_LIST_BASE_URL, DIGIFY_REVOKE_RECIPIENT_DEFAULT_URL } from "./digify-config.mjs";
...
export function digifyFileSearchRequest(context = {}, env = process.env) {
  return digifyFormRequest(
    DIGIFY_FILE_SEARCH_URL,
    {
      SearchQuery: String(context.searchQuery || "").trim(),
      PageIndex: String(Number.isFinite(Number(context.pageIndex)) ? Number(context.pageIndex) : 0),
      PageSize: String(Number.isFinite(Number(context.pageSize)) ? Number(context.pageSize) : 40),
...
export async function searchDigifyFiles(context = {}, env = process.env) {
  const data = await digifyPost(digifyFileSearchRequest(context, env), "Digify file search");
