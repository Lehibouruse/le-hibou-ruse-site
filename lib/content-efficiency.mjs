...
export function contentEfficiencySummary(sales = [], performances = []) {
  ...
  for (const record of performances) {
    ...
    if (!row) continue;
    row.views += Math.max(0, number(fields.Views));
    row.likes += Math.max(0, number(fields.Likes));
    row.comments += Math.max(0, number(fields.Comments));
    row.shares += Math.max(0, number(fields.Shares));
    row.saves += Math.max(0, number(fields.Saves));
  ...
  const rows = [...map.values()].map((row) => {
    ...
    const engagements = row.likes + row.comments + row.shares + row.saves;
    return {
      key: row.key,
      ...
      engagement_rate: row.views > 0 ? Number((engagements / row.views).toFixed(4)) : 0,
      refund_rate: row.purchases > 0 ? Number((row.refunds / row.purchases).toFixed(4)) : 0,
    };
  });
...
export function providerEfficiencySummary(rows = []) {
  ...
  return [...providers.values()].map((row) => ({
    ...
    engagement_rate: row.views > 0 ? Number((row.engagements / row.views).toFixed(4)) : 0,
