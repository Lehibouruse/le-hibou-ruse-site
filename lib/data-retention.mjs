...
function plannedAction(kind, record, classification) {
  if (classification.status === "invalid_date") {
    return {
      table: kind,
      record_id: record?.id || "",
  ...
  if (classification.status === "candidate_for_purge") {
    return {
      table: kind,
      record_id: record?.id || "",
      action: kind === "prospects" ? "purge_or_anonymize_after_review" : "purge_after_review",
      reason: "retention_period_expired",
      age_days: classification.age_days,
    };
  }
  if (classification.status === "legal_archive") {
    return {
      table: kind,
      record_id: record?.id || "",
      action: "keep_legal_archive",
      reason: "accounting_or_contractual_retention",
      age_days: classification.age_days,
    };
  }
  return {
...
export function retentionPlan({ conversionEvents = [], automationLogs = [], prospects = [], sales = [], now = Date.now() } = {}) {
  const groups = [
    ["conversion_events", conversionEvents, classifyConversionEvent],
    ["automation_logs", automationLogs, classifyAutomationLog],
    ["prospects", prospects, classifyProspect],
    ["sales", sales, classifySale],
  ];
  const actions = [];
  for (const [kind, records, classifier] of groups) {
    for (const record of records || []) {
      const classification = classifier(record, now);
      actions.push(plannedAction(kind, record, classification));
    }
  }
  return {
    schema: "HIBOU_RETENTION_PLAN_V1",
    mode: "dry_run",
    destructive_actions: false,
    generated_at: new Date(now).toISOString(),
    policy: RETENTION_POLICY,
    counts: actions.reduce((acc, item) => {
      acc[item.action] = (acc[item.action] || 0) + 1;
      return acc;
    }, {}),
    actions,
    safety: {
      sales_deletion_planned: actions.some((x) => x.table === "sales" && /purge|delete/.test(x.action)),
