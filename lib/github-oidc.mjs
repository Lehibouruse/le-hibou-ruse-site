...
const WORKFLOW_REFS = new Set([
  `${REPOSITORY}/.github/workflows/hibou-wake.yml@${REF}`,
  `${REPOSITORY}/.github/workflows/export-partial-book-once.yml@${REF}`,
]);
...
export function validateGithubActionsClaims(payload, nowSeconds = Math.floor(Date.now() / 1000)) {
  ...
  if (!WORKFLOW_REFS.has(payload.workflow_ref)) throw new Error("OIDC workflow invalide");
  if (!ALLOWED_EVENTS.has(payload.event_name)) throw new Error("OIDC event invalide");
  return payload;
}
