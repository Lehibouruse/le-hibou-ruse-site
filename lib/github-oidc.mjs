...
function workflowRef(workflowFile, ref = REF) {
...
function allowedWorkflowRefs(options = {}) {
  const refs = new Set();
  for (const file of options.allowedWorkflowFiles || []) {
    refs.add(workflowRef(file));
    if (options.allowBranchPush === true && options.actualRef) refs.add(workflowRef(file, options.actualRef));
  }
...
export function validateGithubActionsClaims(payload, nowSeconds = Math.floor(Date.now() / 1000), options = {}) {
  ...
  const branchPushAllowed = options.allowBranchPush === true && payload.event_name === "push" && /^refs\/heads\/[A-Za-z0-9._\/-]+$/.test(String(payload.ref || ""));
  if (payload.ref !== REF && !branchPushAllowed) throw new Error("OIDC ref invalide");
  if (branchPushAllowed) options = { ...options, actualRef: payload.ref };
