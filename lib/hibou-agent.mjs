import { isProjectAiEnabled } from "./ai-policy.mjs";
...
export function getAgentConfig(env = process.env) {
  return {
    aiEnabled: isProjectAiEnabled(env),
    allowTerra: flag(env.HIBOU_ALLOW_TERRA, false),
    allowSol: flag(env.HIBOU_ALLOW_SOL, false),
    allowAstra: flag(env.HIBOU_ALLOW_ASTRA, false),
    maxEscalations: Math.min(2, number(env.HIBOU_MAX_ESCALATIONS, 2, 0)),
    maxApiRetries: Math.min(1, number(env.HIBOU_MAX_API_RETRIES, 0, 0)),
    maxAiCallsPerJob: Math.min(5, number(env.HIBOU_MAX_AI_CALLS_PER_JOB, 0, 0)),
    maxOutputTokens: Math.min(3000, number(env.HIBOU_MAX_OUTPUT_TOKENS, 900, 100)),
    maxCostPerJobUsd: number(env.HIBOU_MAX_COST_PER_JOB_USD, 0, 0),
    dailySoftBudgetUsd: number(env.HIBOU_DAILY_SOFT_BUDGET_USD, 0, 0),
    dailyHardBudgetUsd: number(env.HIBOU_DAILY_HARD_BUDGET_USD, 0, 0),
    models: {
