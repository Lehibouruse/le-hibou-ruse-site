...
export function routeJob(job, config = getAgentConfig()) {
  ...
  if (!AI_ACTIONS.has(action) && !articleNeedsAi) return { kind: "deterministic", action, parameters };
  ...
  let tier = directTier || (articleNeedsAi || INTERMEDIATE_ACTIONS.has(action) ? "terra" : "luna");
