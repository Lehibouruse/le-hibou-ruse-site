...
export function commercialReadiness({ config = {}, product = {}, chapters = [], legal = [], env = process.env } = {}) {
  ...
  const deliveryProvider = text(config.commerce_delivery_provider || "digify").toLowerCase();
  const readerDelivery = deliveryProvider === "hibou_reader";
  ...
  add("delivery_provider", readerDelivery || digifyDelivery, `Provider de livraison reconnu (${deliveryProvider || "absent"})`);
  add(
    "lemon_downloadable_file_removed",
    !readerDelivery || bool(config.lemon_downloadable_file_removed_verified),
    readerDelivery ? "Aucun PDF téléchargeable n'est attaché au variant Lemon live" : "Contrôle de fichier Lemon non requis pour Digify",
    readerDelivery,
  );
  add(
    "secure_reader",
    !readerDelivery || Boolean(text(resolveReaderSecret(env))),
    readerDelivery ? "Secret du lecteur Hibou présent côté serveur" : "Lecteur Hibou non sélectionné",
    readerDelivery,
  );
  ...
  add("digify_status", digifyCommerciallyAvailable, `Digify commercialement disponible et revalidé (${digifyStatus || "statut absent"})`, digifyDelivery);
  add("digify_file", Boolean(text(product["Digify File GUID"])), "PDF maître chargé dans Digify", digifyDelivery);
  ...
  ), "API de livraison, révocation et webhook Digify entièrement configurée", digifyDelivery);
  add(
