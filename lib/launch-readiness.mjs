...
export function commerceTestReadiness({ config = {}, product = {}, env = process.env } = {}) {
  ...
  digifyAdd("add_recipient_endpoint", Boolean(digifyApiUrl(env.DIGIFY_ADD_RECIPIENT_URL)), "Endpoint officiel d'ajout destinataire Digify configuré");
  ...
  digifyAdd("activity_webhook_auth", Boolean(text(env.DIGIFY_WEBHOOK_USERNAME) && text(env.DIGIFY_WEBHOOK_PASSWORD)), "Basic Auth indépendante configurée pour le webhook d'activité Digify");
...
export function commercialReadiness({ config = {}, product = {}, chapters = [], legal = [], env = process.env } = {}) {
  ...
  add("digify_api", Boolean(
    text(env.DIGIFY_KEY_ID)
    && text(env.DIGIFY_SECRET)
    && digifyApiUrl(env.DIGIFY_ADD_RECIPIENT_URL)
    && text(env.DIGIFY_ADD_RECIPIENT_BODY_TEMPLATE)
    && digifyApiUrl(env.DIGIFY_REVOKE_RECIPIENT_URL)
    && text(env.DIGIFY_REVOKE_RECIPIENT_BODY_TEMPLATE)
  ), "API de livraison et révocation Digify entièrement configurée");
  add(
