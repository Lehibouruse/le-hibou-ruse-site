...
export function commercialReadiness({ config = {}, product = {}, chapters = [], legal = [], env = process.env } = {}) {
  ...
  const deliveryProvider = text(config.delivery_provider_mode || "digify").toLowerCase();
  const deliveryProviderKnown = ["digify", "lemon_native", "hibou_reader"].includes(deliveryProvider);
  const digifyRuntime = digifyReadiness(env);
  const digifyStatus = text(config.digify_api_status).toUpperCase();
  const digifyCommerciallyAvailable = ["ACTIVE", "READY", "LIVE", "LIVE_DELIVERABLE", "DELIVERABLE"].includes(digifyStatus);
  const digifyApiReady = Boolean(
  ...
  const lemonNativeVerified = bool(config.lemon_native_delivery_verified);
  const readerDownloadRemoved = bool(config.lemon_downloadable_file_removed_verified);
  const readerSecretReady = Boolean(text(resolveReaderSecret(env)));
  ...
  add("delivery_provider", deliveryProviderKnown, `Provider de livraison reconnu (${deliveryProvider || "absent"})`);
  add("digify_status", deliveryProvider !== "digify" || digifyCommerciallyAvailable, `Digify commercialement disponible et revalidé (${digifyStatus || "statut absent"})`, deliveryProvider === "digify");
  add("digify_file", deliveryProvider !== "digify" || Boolean(text(product["Digify File GUID"])), "PDF maître chargé dans Digify", deliveryProvider === "digify");
  add("digify_api", deliveryProvider !== "digify" || digifyApiReady, "API de livraison, révocation et webhook Digify entièrement configurée", deliveryProvider === "digify");
  add("lemon_native_delivery", deliveryProvider !== "lemon_native" || lemonNativeVerified, "Livraison native Lemon vérifiée sur le variant LIVE", deliveryProvider === "lemon_native");
  add(
    "lemon_downloadable_file_removed",
    deliveryProvider !== "hibou_reader" || readerDownloadRemoved,
    deliveryProvider === "hibou_reader" ? "Aucun PDF téléchargeable attaché au variant Lemon LIVE" : "Retrait du fichier Lemon non requis pour ce provider",
    deliveryProvider === "hibou_reader",
  );
  add(
    "secure_reader",
    deliveryProvider !== "hibou_reader" || readerSecretReady,
    deliveryProvider === "hibou_reader" ? "Secret serveur du lecteur Hibou disponible" : "Lecteur Hibou non sélectionné",
    deliveryProvider === "hibou_reader",
  );
  add(
