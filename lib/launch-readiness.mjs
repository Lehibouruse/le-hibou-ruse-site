...
export function commerceTestReadiness({ config = {}, product = {}, env = process.env } = {}) {
  ...
  digifyAdd("add_recipient_template", digifyRuntime.add_template_present, "Payload officiel d'ajout destinataire configuré");
  ...
  digifyAdd("revoke_template", digifyRuntime.revoke_template_present, "Payload officiel de révocation configuré");
...
export function commercialReadiness({ config = {}, product = {}, chapters = [], legal = [], env = process.env } = {}) {
  ...
  add("digify_api", Boolean(
    ...
    && digifyRuntime.add_template_present
    && digifyApiUrl(env.DIGIFY_REVOKE_RECIPIENT_URL)
    && digifyRuntime.revoke_template_present
