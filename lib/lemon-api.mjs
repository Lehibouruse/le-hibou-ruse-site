...
export async function listLemonFiles(variantId, options = {}) {
  const data = await lemonRequest("/v1/files", {
    ...options,
    query: { "filter[variant_id]": requireId(variantId, "Variant ID"), "page[size]": 100, ...(options.query || {}) },
  });
  return Array.isArray(data?.data) ? data.data : [];
}

export function summarizeLemonFiles(files = []) {
  return (Array.isArray(files) ? files : []).map((file) => ({
    id: text(file?.id),
    name: text(file?.attributes?.name),
    extension: text(file?.attributes?.extension),
    size: Number(file?.attributes?.size || 0),
    size_formatted: text(file?.attributes?.size_formatted),
    version: text(file?.attributes?.version),
    status: text(file?.attributes?.status),
    test_mode: file?.attributes?.test_mode === true,
  }));
}
  ...
  return {
    data: {
      ...
      attributes: {
        ...
        checkout_data: {
          custom: Object.fromEntries(Object.entries(checkoutCustomData || {})
            .filter(([key, value]) => text(key) && value !== undefined && value !== null && text(value))
            .map(([key, value]) => [text(key).slice(0, 120), text(value).slice(0, 500)])),
        },
      },
      relationships: {
...
export function buildLiveCheckoutPayload(input = {}) {
  ...
  if (!/édition numérique/i.test(description) || !/(mise[s]? à jour|disponible)/i.test(description)) {
    throw new Error("Checkout live: description transparente de l’édition numérique requise");
  }
