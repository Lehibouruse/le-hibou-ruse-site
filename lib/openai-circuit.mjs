...
export function creditPausePatch(fields = {}, circuit = {}, message = "") {
  const untilMs = Date.parse(text(circuit.until));
  if (!Number.isFinite(untilMs)) throw new Error("Échéance circuit OpenAI invalide");
  return {
    status: "Retry",
    retry_count: Math.max(0, Number(fields.retry_count || 0)),
    next_run_at: new Date(untilMs).toISOString(),
    completed_at: null,
    error: text(message).slice(0, 5000),
    agent_status: "waiting_for_credit",
    lock_token: "",
