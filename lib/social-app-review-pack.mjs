...
const COMMON = {
  ...
  website: "https://le-hibou-ruse-site.vercel.app",
...
const PROVIDERS = {
  ...
  meta: {
    ...
    use_case: "Publish editorial content to the Facebook Page and Instagram professional account administered by Le Hibou Rusé, verify the linked Page/Instagram account, and retrieve first-party post/media insights.",
    permissions: "Page listing is used to select the Page administered by Le Hibou Rusé. Page publishing permissions are used only for that Page. Instagram basic/content-publish permissions are used only for the linked professional Instagram account. Page and Instagram insight permissions are used to measure the performance of Le Hibou Rusé content.",
    validation: "Payloads can be prepared and dry-run without posting. Because a real Meta post may be visible publicly, no live technical test is executed without explicit human approval.",
  },
  tiktok: {
    label: "TikTok",
