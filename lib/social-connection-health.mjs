...
const SUPPORTED_NETWORKS = ["youtube", "instagram", "facebook", "tiktok", "linkedin", "threads", "x"];

function clean(value) { return String(value ?? "").trim(); }
function escFormula(value) { return clean(value).replaceAll("\\", "\\\\").replaceAll("'", "\\'"); }
function platformLabel(network) {
  return ({ youtube: "YouTube", instagram: "Instagram", facebook: "Facebook", tiktok: "TikTok", linkedin: "LinkedIn", threads: "Threads", x: "X" })[network] || network;
}
...
function nextHumanAction(network) {
  ...
  if (network === "linkedin") return "Après lecture API : effectuer un test de publication ; l’analytics avancée dépend de l’accès Community Management.";
...
async function markFailure(network, error, { oauthConnected = false } = {}) {
  ...
  await updateRecord(TABLES.socialAccounts, row.id, {
    ...
    "État direct": reauth ? "NEEDS_REAUTH" : "AUTH_ERROR",
    "Dernier test API": new Date().toISOString(),
    "Dernière vérification": new Date().toISOString(),
    "Blocage API direct": String(error?.message || error).slice(0, 1500),
    "Action humaine restante": reauth ? "Reconnecter le compte via /admin/social puis relancer le test API." : "Corriger la configuration/scopes de l’app puis relancer le test API.",
  });
  return true;
}

async function probeYoutube(env, scopes, fetchImpl) {
  const token = clean(env.YOUTUBE_ACCESS_TOKEN); if (!token) throw new Error("YOUTUBE_ACCESS_TOKEN absent");
  ...
  const channel = data?.items?.[0]; if (!channel?.id) throw new Error("YouTube: aucun canal authentifié retourné");
  return { scopes, technicalId: channel.id, publicId: clean(channel.snippet?.customUrl || channel.snippet?.title), profileUrl: channel.snippet?.customUrl ? `https://youtube.com/${channel.snippet.customUrl}` : `https://youtube.com/channel/${channel.id}` };
}

async function probeTikTok(env, scopes, fetchImpl) {
  const token = clean(env.TIKTOK_ACCESS_TOKEN); if (!token) throw new Error("TIKTOK_ACCESS_TOKEN absent");
  ...
  const user = data?.data?.user; if (!user?.open_id) throw new Error("TikTok: utilisateur authentifié absent");
...
async function probeX(env, scopes, fetchImpl) {
  const token = clean(env.X_ACCESS_TOKEN); if (!token) throw new Error("X_ACCESS_TOKEN absent");
  const url = new URL("https://api.x.com/2/users/me"); url.searchParams.set("user.fields", "id,name,username");
  const data = await readJson(await fetchImpl(url, { headers: { Authorization: `Bearer ${token}` }, cache: "no-store" }), "X identity");
  const user = data?.data; if (!user?.id) throw new Error("X: utilisateur authentifié absent");
...
async function probeLinkedIn(env, scopes, fetchImpl) {
  const token = clean(env.LINKEDIN_ACCESS_TOKEN); if (!token) throw new Error("LINKEDIN_ACCESS_TOKEN absent");
...
async function probeThreads(env, scopes, fetchImpl) {
  const token = clean(env.THREADS_ACCESS_TOKEN); if (!token) throw new Error("THREADS_ACCESS_TOKEN absent");
  const url = new URL("https://graph.threads.net/v1.0/me"); url.searchParams.set("fields", "id,username,name"); url.searchParams.set("access_token", token);
...
async function probeMetaNetwork(network, env, scopes, fetchImpl) {
  const token = clean(env.FACEBOOK_PAGE_ACCESS_TOKEN || env.META_ACCESS_TOKEN); if (!token) throw new Error("META/FACEBOOK access token absent");
  if (network === "facebook") {
    const pageId = clean(env.FACEBOOK_PAGE_ID); if (!pageId) throw new Error("FACEBOOK_PAGE_ID absent");
    const url = new URL(`https://graph.facebook.com/${encodeURIComponent(pageId)}`); url.searchParams.set("fields", "id,name,link"); url.searchParams.set("access_token", token);
  ...
  const accountId = clean(env.INSTAGRAM_BUSINESS_ACCOUNT_ID); if (!accountId) throw new Error("INSTAGRAM_BUSINESS_ACCOUNT_ID absent");
  const url = new URL(`https://graph.facebook.com/${encodeURIComponent(accountId)}`); url.searchParams.set("fields", "id,username,name,profile_picture_url"); url.searchParams.set("access_token", token);
...
export async function testSocialConnection(networkInput, baseEnv = process.env, fetchImpl = fetch) {
  ...
  try {
    ...
    return { provider: network, ok: true, state: "READ_TESTED", identity: { technical_id: identity.technicalId, public_id: identity.publicId, profile_url: identity.profileUrl }, credential_source: resolved.source };
  } catch (error) {
    await markFailure(network, error, { oauthConnected: true });
    return { provider: network, ok: false, state: [401, 403].includes(Number(error?.status || 0)) ? "NEEDS_REAUTH" : "AUTH_ERROR", error: String(error?.message || error).slice(0, 500) };
  }
}
...
export async function testVaultProviderConnections(vaultProvider, baseEnv = process.env, fetchImpl = fetch) {
  ...
  const results = [];
  for (const network of networks.filter((item) => SUPPORTED_NETWORKS.includes(item))) results.push(await testSocialConnection(network, baseEnv, fetchImpl));
  return results;
}
