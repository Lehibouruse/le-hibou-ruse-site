...
function nextHumanAction(network) {
  ...
  if (network === "reddit") return "Après lecture API : choisir un subreddit dont les règles autorisent le contenu, puis effectuer uniquement un test de publication humainement validé.";
...
async function probeReddit(env, scopes, fetchImpl) {
  const identity = await redditIdentity(env, fetchImpl);
  return { scopes, technicalId: identity.technicalId, publicId: identity.publicId, profileUrl: identity.profileUrl };
}
...
async function probe(network, resolved, fetchImpl) {
  ...
  if (network === "reddit") return probeReddit(resolved.env, scopes, fetchImpl);
