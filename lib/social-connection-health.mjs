...
function nextHumanAction(network) {
  ...
  if (network === "reddit") return "Après accord Reddit et lecture API validée : choisir une communauté autorisant le contenu, puis effectuer une publication contrôlée uniquement avec validation humaine.";
...
export async function testSocialConnection(networkInput, baseEnv = process.env, fetchImpl = fetch) {
  ...
  if (network === "reddit" && clean(baseEnv.REDDIT_API_APPROVED).toLowerCase() !== "true") {
    return {
      provider: "reddit",
      ok: false,
      state: "EXTERNAL_APPROVAL_REQUIRED",
      error: "Reddit : approbation API commerciale requise avant tout appel API.",
    };
  }
