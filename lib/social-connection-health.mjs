...
import { redditIdentity } from "./social-reddit.mjs";

const SUPPORTED_NETWORKS = ["youtube", "instagram", "facebook", "tiktok", "linkedin", "threads", "pinterest", "x", "bluesky", "reddit"];
...
function platformLabel(network) {
  return ({
    ...
    linkedin: "LinkedIn", threads: "Threads", pinterest: "Pinterest", x: "X", bluesky: "Bluesky", reddit: "Reddit",
  })[network] || network;
}
...
function nextHumanAction(network) {
  ...
  if (network === "linkedin") return "Lecture Page validée ; LinkedIn n’offre pas de publication Page privée équivalente au SELF_ONLY : le premier post réel reste bloqué jusqu’à validation humaine explicite.";
...
async function markSuccess(network, { scopes = "", technicalId = "", publicId = "", profileUrl = "" } = {}) {
  ...
  const fields = {
    ...
    "OAuth Hibou connecté": network !== "bluesky",
    "Auth direct OK": true,
    "Lecture API OK": true,
...
async function markFailure(network, error, { oauthConnected = false } = {}) {
  ...
  await updateRecord(TABLES.socialAccounts, row.id, {
    "OAuth Hibou connecté": network !== "bluesky" && oauthConnected && !reauth,
    "Direct Hibou prêt": false,
    ...
    "Action humaine restante": network === "bluesky"
      ? "Remplacer BLUESKY_APP_PASSWORD par le mot de passe d’application complet dans Vercel Production, vérifier BLUESKY_IDENTIFIER, redéployer puis relancer le test Bluesky."
      : reauth
...
async function probeLinkedIn(env, scopes, fetchImpl) {
  ...
  if (!organizationUrn) throw new Error("LINKEDIN_ORGANIZATION_URN absent");
  if (!version) throw new Error("LINKEDIN_VERSION absent");
  ...
  if (!Array.isArray(data?.elements)) throw new Error("LinkedIn: lecture Page non confirmée");
  const organizationId = organizationUrn.split(":").pop() || organizationUrn;
  return { scopes, technicalId: organizationUrn, publicId: organizationId, profileUrl: organizationId ? `https://www.linkedin.com/company/${encodeURIComponent(organizationId)}` : "" };
}
...
async function probeInstagram(env, scopes, fetchImpl) {
  const token = clean(env.INSTAGRAM_ACCESS_TOKEN);
  if (!token) throw new Error("INSTAGRAM_ACCESS_TOKEN absent");
  ...
  const url = new URL(`https://graph.instagram.com/${version}/${encodeURIComponent(accountId)}`);
...
async function probeBluesky(env, scopes, fetchImpl) {
  const identifier = clean(env.BLUESKY_IDENTIFIER);
  const password = clean(env.BLUESKY_APP_PASSWORD);
  if (!identifier || !password) throw new Error("BLUESKY_IDENTIFIER/BLUESKY_APP_PASSWORD absents");
  const rawBase = clean(env.BLUESKY_PDS_URL) || "https://bsky.social";
  const base = new URL(rawBase);
  if (base.protocol !== "https:") throw new Error("BLUESKY_PDS_URL doit utiliser HTTPS");
  const response = await fetchImpl(`${base.origin}/xrpc/com.atproto.server.createSession`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ identifier, password }),
    cache: "no-store",
  });
  const data = await readJson(response, "Bluesky identity");
  if (!data?.did || !data?.handle) throw new Error("Bluesky: identité authentifiée absente");
  return {
    scopes,
    technicalId: data.did,
    publicId: clean(data.handle),
...
async function probeMetaNetwork(network, env, scopes, fetchImpl) {
  ...
  throw new Error("Meta network non pris en charge");
}

async function probe(network, resolved, fetchImpl) {
  const scopes = clean(resolved.scopes);
  if (network === "reddit") return { ...await redditIdentity(resolved.env, fetchImpl), scopes };
  ...
  if (network === "instagram") return probeInstagram(resolved.env, scopes, fetchImpl);
  if (network === "facebook") return probeMetaNetwork(network, resolved.env, scopes, fetchImpl);
  if (network === "bluesky") return probeBluesky(resolved.env, scopes, fetchImpl);
...
export async function testSocialConnection(networkInput, baseEnv = process.env, fetchImpl = fetch) {
  ...
  const environmentConnected = network === "bluesky" && Boolean(clean(resolved.env.BLUESKY_IDENTIFIER)) && Boolean(clean(resolved.env.BLUESKY_APP_PASSWORD));
  const credentialConnected = oauthConnected || environmentConnected;
  ...
  try {
    ...
    await markFailure(network, error, { oauthConnected: credentialConnected });
    return {
      provider: network,
      ok: false,
...
export async function testVaultProviderConnections(vaultProvider, baseEnv = process.env, fetchImpl = fetch) {
  ...
  const networks = provider === "meta" ? ["facebook"] : [provider];
  const results = [];
