...
const PROVIDERS = {
  reddit: {
    label: "Reddit", airtablePlatforms: ["Reddit"],
    ...
    fallbackNetworks: ["reddit"], metricoolVerified: false,
    externalPhase: "EXTERNAL_REDDIT_API_APPROVAL_REQUIRED",
    externalBlocker: "Obtenir l’accord Reddit pour l’usage commercial, puis configurer l’app OAuth Web et ses identifiants. REDDIT_API_APPROVED doit rester absent tant que l’accord n’est pas obtenu.",
    sessionStep: "Demande Reddit → accord → app Web et callback affiché → Client ID/Secret et User-Agent dans Vercel → consentement du compte Hibou → test de lecture. Choisir ensuite un subreddit autorisant le contenu avant tout test de publication.",
  },
  youtube: {
    label: "YouTube",
...
function present(name, env) {
  if (name === "REDDIT_API_APPROVED") return clean(env?.[name]) === "true";
