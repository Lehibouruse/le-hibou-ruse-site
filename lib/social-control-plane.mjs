...
const PROVIDERS = {
  reddit: {
    label: "Reddit",
    airtablePlatforms: ["Reddit"],
    portal: "https://support.reddithelp.com/hc/en-us/articles/14945211791892-Developer-Platform-Accessing-Reddit-Data",
    requiredEnv: ["HIBOU_SOCIAL_VAULT_KEY", "REDDIT_API_APPROVED", "REDDIT_CLIENT_ID", "REDDIT_CLIENT_SECRET", "REDDIT_USER_AGENT"],
    optionalEnv: ["REDDIT_OAUTH_SCOPES", "REDDIT_EXPECTED_USERNAME", "REDDIT_SUBREDDIT"],
    fallbackNetworks: ["reddit"],
    metricoolVerified: false,
    externalPhase: "EXTERNAL_REDDIT_API_APPROVAL_REQUIRED",
    externalBlocker: "Obtenir l’approbation explicite de Reddit pour l’usage commercial avant tout OAuth ou appel Data API. REDDIT_API_APPROVED doit rester false jusque-là.",
    sessionStep: "Après accord Reddit seulement : configurer l’app OAuth Web, stocker Client ID/Secret et User-Agent côté serveur, autoriser le compte Hibou, tester l’identité puis choisir un subreddit autorisant le contenu avant toute publication.",
  },
  youtube: {
    label: "YouTube",
  ...
  meta: {
    label: "Meta · Facebook",
    airtablePlatforms: ["Facebook"],
    portal: "https://developers.facebook.com/apps/",
    requiredEnv: ["HIBOU_SOCIAL_VAULT_KEY", "META_APP_ID", "META_APP_SECRET"],
    optionalEnv: ["META_OAUTH_SCOPES", "META_TARGET_PAGE_ID"],
    fallbackNetworks: ["facebook"],
    metricoolVerified: true,
    externalPhase: "EXTERNAL_META_APP_SETUP_REQUIRED",
    externalBlocker: "Configurer Facebook Login for Business avec pages_show_list, pages_manage_posts, pages_read_engagement et read_insights. Instagram utilise désormais un flux Business Login séparé afin d’éviter les scopes invalides dans le dialogue Facebook.",
    sessionStep: "Meta for Developers → Facebook Login for Business → callback Hibou → permissions Page → App ID/Secret généraux dans Vercel → OAuth Facebook. Instagram est traité séparément via Instagram Business Login.",
  },
  instagram: {
    label: "Instagram",
    airtablePlatforms: ["Instagram"],
    portal: "https://developers.facebook.com/apps/",
    requiredEnv: ["HIBOU_SOCIAL_VAULT_KEY", "INSTAGRAM_APP_ID", "INSTAGRAM_APP_SECRET"],
    optionalEnv: ["INSTAGRAM_OAUTH_SCOPES", "INSTAGRAM_GRAPH_VERSION"],
    fallbackNetworks: ["instagram"],
    metricoolVerified: true,
    externalPhase: "EXTERNAL_INSTAGRAM_BUSINESS_LOGIN_SETUP_REQUIRED",
    externalBlocker: "Configurer Instagram API with Instagram Login dans Meta for Developers, déclarer le callback Hibou et les permissions instagram_business_basic, instagram_business_content_publish et instagram_business_manage_insights.",
    sessionStep: "Meta for Developers → Instagram → API setup with Instagram login → Business Login settings → callback Hibou → Instagram App ID/Secret dans Vercel → OAuth Instagram.",
  },
  tiktok: {
    label: "TikTok",
  ...
  linkedin: {
    ...
    metricoolVerified: true,
    manualOnly: true,
    externalPhase: "MANUAL_ONLY_BY_PRODUCT_DECISION",
    externalBlocker: "Décision produit du 26/09/2026 : LinkedIn reste en publication manuelle au lancement. Le chantier Community Management API ne doit pas être relancé automatiquement.",
    sessionStep: "Publication manuelle LinkedIn. Ne reprendre le chantier API que sur demande explicite de Marc.",
  },
  pinterest: {
    label: "Pinterest",
    ...
    manualOnly: true,
    externalPhase: "MANUAL_ONLY_BY_PRODUCT_DECISION",
    externalBlocker: "Décision produit du 26/09/2026 : Pinterest reste en publication manuelle au lancement ; ne pas relancer Trial/Standard Access ou OAuth automatiquement.",
    sessionStep: "Publication manuelle Pinterest. Ne reprendre le chantier API que sur demande explicite de Marc.",
  },
  x: {
    label: "X",
    airtablePlatforms: ["X"],
    ...
    manualOnly: true,
    externalPhase: "MANUAL_ONLY_BY_PRODUCT_DECISION",
    externalBlocker: "Décision produit du 26/09/2026 : X reste en publication manuelle au lancement. Ne déclencher aucun achat, crédit ou chantier API automatiquement.",
    sessionStep: "Publication manuelle X. Ne reprendre l’API qu’après demande explicite de Marc et validation explicite de toute dépense.",
  },
  threads: {
    label: "Threads",
...
function present(name, env) {
  if (name === "REDDIT_API_APPROVED") return clean(env?.[name]).toLowerCase() === "true";
...
function currentExternalBlocker(provider, grant, meta, phase, env = process.env) {
  if (phase.startsWith("EXTERNAL_")) return meta.externalBlocker || grant.blocker || "";
  if (provider === "tiktok" && clean(env.TIKTOK_APP_AUDITED).toLowerCase() !== "true") {
    return "TikTok : authentification/scopes acquis, mais publication directe publique reste soumise à l’audit TikTok. Conserver SELF_ONLY / test contrôlé jusqu’à TIKTOK_APP_AUDITED=true.";
  }
  return "";
}

function currentAuthorizationStep(provider, meta, phase, env = process.env) {
  if (phase.startsWith("EXTERNAL_") || phase === "HUMAN_OAUTH_APPROVAL_REQUIRED" || phase === "PUBLISH_SCOPE_REVIEW_REQUIRED" || phase === "ANALYTICS_SCOPE_REVIEW_REQUIRED") {
    return meta.sessionStep || "";
  }
  if (provider === "tiktok" && clean(env.TIKTOK_APP_AUDITED).toLowerCase() !== "true") {
    return "Terminer l’audit TikTok avant le public ; jusque-là limiter les essais directs au mode autorisé/SELF_ONLY.";
  }
  return "";
}
...
export function buildSocialControlPlane({ readiness = [], credentials = [], env = process.env } = {}) {
  ...
  const providers = grants.map((grant) => {
    ...
    return {
      ...
      external_blocker: currentExternalBlocker(grant.provider, grant, meta, phase, env),
      authorization_session_step: currentAuthorizationStep(grant.provider, meta, phase, env),
