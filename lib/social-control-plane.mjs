...
const PROVIDERS = {
  youtube: {
    ...
    externalBlocker: "Créer/configurer le projet Google Cloud du Hibou, activer YouTube Data API v3, créer un client OAuth Web et y déclarer exactement le callback Hibou. Une fois Client ID/Secret placés côté Vercel, le réseau passe à HUMAN_OAUTH_APPROVAL_REQUIRED. Une vérification Google peut ensuite être requise pour une application publique utilisant des scopes YouTube sensibles.",
    sessionStep: "Google Cloud → YouTube Data API v3 → écran de consentement OAuth → application Web → ajouter le callback affiché → placer Client ID/Secret dans Vercel → revenir au bouton Valider l’autorisation.",
  },
  meta: {
    ...
    externalBlocker: "Créer/configurer l’app Meta du Hibou, déclarer le callback, rattacher la Page Facebook au compte Instagram professionnel et activer les produits/scopes requis. Les permissions avancées/app review peuvent être nécessaires pour un usage live au-delà des rôles de l’app. Après App ID/Secret côté Vercel, l’étape suivante est HUMAN_OAUTH_APPROVAL_REQUIRED.",
    sessionStep: "Meta for Developers → app Hibou → configurer Facebook/Instagram + callback → vérifier liaison Page Facebook ↔ Instagram professionnel → placer App ID/Secret et version Graph dans Vercel → lancer l’OAuth.",
  },
  tiktok: {
    label: "TikTok",
    ...
    externalBlocker: "Créer/configurer l’app TikTok, ajouter Login Kit et Content Posting API, enregistrer le callback et obtenir les scopes nécessaires. Tant que l’app n’est pas auditée, la publication directe publique reste limitée et SELF_ONLY doit être utilisé. Après Client Key/Secret côté Vercel, le consentement OAuth utilisateur devient possible ; l’audit TikTok reste le blocage externe du public.",
    sessionStep: "TikTok for Developers → app Hibou → Login Kit + Content Posting API → callback → Client Key/Secret dans Vercel → OAuth. Les tests directs restent SELF_ONLY jusqu’à audit/approbation TikTok.",
  },
  linkedin: {
    label: "LinkedIn",
    ...
    externalBlocker: "Pour un pilotage direct de la Page/organisation LinkedIn, l’app doit obtenir les produits/permissions appropriés, notamment Community Management API pour w_organization_social et les analytics. Le membre doit aussi avoir le rôle adéquat sur la Page. L’identifiant organisation est déjà connu. Après Client ID/Secret et produits autorisés, l’OAuth utilisateur devient possible.",
    sessionStep: "LinkedIn Developers → app Hibou → demander/activer les produits nécessaires (Share on LinkedIn ; Community Management pour Page + analytics) → callback → Client ID/Secret dans Vercel → OAuth avec le compte administrateur de l’organisation.",
  },
  pinterest: {
    label: "Pinterest",
    ...
    externalBlocker: "Pinterest exige l’enregistrement de l’app et l’approbation Trial Access avant l’accès API normal. Le Trial permet les tests et les Pins/boards créés restent visibles seulement par le créateur ; Standard Access sera requis pour un usage de production complet. Après App ID/Secret + Trial, l’OAuth Hibou peut être validé.",
    sessionStep: "Pinterest Developers → Connect app → demander Trial Access → déclarer callback → placer App ID/Secret dans Vercel → OAuth. Plus tard, demander Standard Access avec démo OAuth pour la production.",
  },
  x: {
    label: "X",
    airtablePlatforms: ["X"],
  ...
  threads: {
    ...
    externalBlocker: "Créer/configurer l’app Threads chez Meta, enregistrer le callback et activer les permissions threads_basic, threads_content_publish et threads_manage_insights. Après App ID/Secret côté Vercel, la prochaine étape devient le consentement OAuth personnel.",
