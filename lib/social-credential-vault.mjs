...
const PROVIDERS = new Set(["youtube", "tiktok", "meta", "linkedin", "x", "threads", "pinterest"]);
...
function keyFromEnv(env = process.env) {
  ...
  if (!raw) throw new Error("HIBOU_SOCIAL_VAULT_KEY absent");
  let key;
  if (/^[0-9a-f]{64}$/i.test(raw)) key = Buffer.from(raw, "hex");
  else {
    try { key = Buffer.from(raw.replace(/-/g, "+").replace(/_/g, "/"), "base64"); } catch {}
  }
  if (!key || key.length !== 32) throw new Error("HIBOU_SOCIAL_VAULT_KEY doit représenter exactement 32 octets");
  return key;
}
...
function decode(value) { return Buffer.from(clean(value), "base64url"); }
...
export function encryptSocialCredential(providerInput, accountKeyInput, payload, env = process.env) {
  ...
  return { provider, accountKey, ciphertext: encode(ciphertext), iv: encode(iv), authTag: encode(cipher.getAuthTag()), version: VERSION };
}
...
export function decryptSocialCredential(recordOrFields, env = process.env) {
  ...
  const plaintext = Buffer.concat([decipher.update(decode(fields.Ciphertext)), decipher.final()]).toString("utf8");
...
async function findRecord(provider, accountKey = "primary") {
  ...
  const records = await queryRecords(TABLES.socialCredentials, {
    filterByFormula: `AND({Provider}='${normalizedProvider}',{Account key}='${normalizedAccount}')`, pageSize: 2,
  });
...
export async function storeSocialCredential({ provider, accountKey = "primary", payload, scopes = "", expiresAt = null, status = "Connected", error = "" }, env = process.env) {
  ...
  const fields = {
    Provider: encrypted.provider, "Account key": encrypted.accountKey, Ciphertext: encrypted.ciphertext, IV: encrypted.iv,
    "Auth tag": encrypted.authTag, Scopes: clean(scopes), "Expires at": expiresAt || null, Status: status,
    "Updated at": new Date().toISOString(), "Last error": clean(error).slice(0, 4000), "Vault version": VERSION,
  };
  const existing = await findRecord(encrypted.provider, encrypted.accountKey);
  if (existing) { await updateRecord(TABLES.socialCredentials, existing.id, fields); return existing.id; }
...
export async function loadSocialCredential(provider, accountKey = "primary", env = process.env) {
  ...
  return {
    id: record.id, provider: record.fields.Provider, accountKey: record.fields["Account key"] || "primary",
    scopes: clean(record.fields.Scopes), expiresAt: record.fields["Expires at"] || null,
...
export async function socialCredentialStatuses() {
  ...
  return records.map((record) => ({
    provider: clean(record.fields?.Provider), account_key: clean(record.fields?.["Account key"] || "primary"),
    status: record.fields?.Status?.name || record.fields?.Status || "", scopes: clean(record.fields?.Scopes),
    expires_at: record.fields?.["Expires at"] || null, updated_at: record.fields?.["Updated at"] || null,
    last_error: clean(record.fields?.["Last error"]), vault_version: clean(record.fields?.["Vault version"]),
  }));
}
