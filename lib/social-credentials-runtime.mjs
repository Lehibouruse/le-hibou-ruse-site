...
import { redditToken, requireRedditApproval } from "./social-reddit.mjs";

const NETWORKS = ["youtube", "instagram", "facebook", "tiktok", "linkedin", "threads", "pinterest", "x", "bluesky", "reddit", "snapchat"];
const REFRESH_WINDOW_MS = 10 * 60 * 1000;

const ALLOWED_KEYS = {
  reddit: ["REDDIT_ACCESS_TOKEN", "REDDIT_REFRESH_TOKEN", "REDDIT_USERNAME"],
...
function vaultProvider(network) {
  ...
  if (["youtube", "instagram", "tiktok", "linkedin", "threads", "pinterest", "x", "reddit"].includes(value)) return value;
  return "";
}
...
async function refreshCredential(provider, credential, baseEnv) {
  ...
  if (provider === "reddit") {
    if (!clean(current.REDDIT_REFRESH_TOKEN)) return markNeedsReauth(provider, credential, "Reddit refresh token absent", baseEnv);
    const token = await redditToken({ grant_type: "refresh_token", refresh_token: current.REDDIT_REFRESH_TOKEN }, baseEnv);
    return persistRefreshed(provider, credential, {
      REDDIT_ACCESS_TOKEN: token.access_token,
      REDDIT_REFRESH_TOKEN: token.refresh_token || current.REDDIT_REFRESH_TOKEN,
    }, token, baseEnv);
  }
  ...
  if (provider === "tiktok") {
    ...
    const token = requireTikTokUserToken(await tokenForm("https://open.tiktokapis.com/v2/oauth/token/", {
...
export async function resolveSocialEnv(network, baseEnv = process.env) {
  ...
  let credential = null;
  try {
    if (provider === "reddit") requireRedditApproval(baseEnv);
