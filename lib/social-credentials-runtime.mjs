...
async function tokenForm(url, fields, headers = {}) {
  ...
  if (!response.ok) throw new Error(`OAuth refresh ${response.status}: ${data.error_description || data.error || data.text || response.statusText}`);
  return data;
}
...
async function refreshCredential(provider, credential, baseEnv) {
  ...
  if (provider === "threads") {
    ...
    if (!response.ok || !token.access_token) throw new Error(`Threads refresh ${response.status}: ${token.error?.message || token.error || response.statusText}`);
...
export async function resolveSocialEnv(network, baseEnv = process.env) {
  ...
  try {
    let credential = await loadSocialCredential(provider, "primary", baseEnv);
