...
import { reconcileTikTokDispatch } from "./tiktok-dispatch-confirmation.mjs";
...
function tiktokConfirmationOptions(env = process.env) {
  const attempts = Number(env.TIKTOK_STATUS_POLL_ATTEMPTS || 4);
  const intervalMs = Number(env.TIKTOK_STATUS_POLL_INTERVAL_MS || 1500);
  return { attempts, intervalMs };
}

async function reconcileTikTokIfDirect(result, env) {
  return reconcileTikTokDispatch(result, clean(env.TIKTOK_ACCESS_TOKEN), tiktokConfirmationOptions(env));
}
...
export async function dispatchSocialPost(input, env = process.env) {
  ...
  if (provider === "tiktok" && !tiktokPhotoRequested(input)) {
    const dispatched = await dispatchEnhanced(input, env);
    return reconcileTikTokIfDirect(dispatched, env);
  }

  if (provider !== "tiktok") return dispatchEnhanced(input, env);
  ...
  const photo = await dispatchTikTokPhoto(input, env);
  const dispatched = { ok: true, dry_run: false, provider: "tiktok", mode: "direct", result: photo };
  return reconcileTikTokIfDirect(dispatched, env);
}
