import { dispatchReddit, requireRedditApproval } from "./social-reddit.mjs";
const PROVIDERS = ["youtube", "instagram", "facebook", "tiktok", "linkedin", "threads", "pinterest", "x", "bluesky", "reddit", "snapchat"];
...
function directConfigured(provider, env = process.env) {
  ...
  if (provider === "reddit") return clean(env.REDDIT_API_APPROVED).toLowerCase() === "true" && hasAll(env, ["REDDIT_ACCESS_TOKEN", "REDDIT_USER_AGENT", "REDDIT_SUBREDDIT"]);
...
function missingDirectEnv(provider, env = process.env) {
  ...
  if (provider === "youtube") {
    ...
    if (clean(env.REDDIT_API_APPROVED).toLowerCase() !== "true") missing.push("REDDIT_API_APPROVED");
    for (const key of ["REDDIT_ACCESS_TOKEN", "REDDIT_USER_AGENT", "REDDIT_SUBREDDIT"]) if (!clean(env[key])) missing.push(key);
...
function directCapabilities(provider) {
  ...
  if (provider === "reddit") return ["text_native", "link_native"];
...
export function socialGatewayStatus(env = process.env) {
  return PROVIDERS.map((provider) => {
    const webhook = Boolean(webhookUrl(provider, env));
    const direct = directConfigured(provider, env);
    const approved = provider !== "reddit" || clean(env.REDDIT_API_APPROVED).toLowerCase() === "true";
    const mode = approved ? resolvedMode(provider, env) : "unconfigured";
    const constraints = [];
    if (!approved) constraints.push("Reddit commercial API approval required");
    ...
    return {
      provider,
      configured: approved && (webhook || direct),
      mode,
      ...
      live_publish_supported: approved && (webhook || (direct && provider !== "snapchat")),
...
async function dispatchDirect(provider, payload, env) {
  ...
  if (provider === "reddit") return dispatchReddit(payload, env);
...
export async function dispatchSocialPost(input, env = process.env) {
  ...
  if (!rawMediaUrl && !["bluesky", "reddit"].includes(provider)) throw new Error("media_url est requis pour ce réseau");
  ...
  if (provider === "reddit") requireRedditApproval(env);
