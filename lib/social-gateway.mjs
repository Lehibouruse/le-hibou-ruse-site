...
function directConfigured(provider, env = process.env) {
  ...
  if (provider === "reddit") return clean(env.REDDIT_API_APPROVED) === "true" && hasAll(env, ["REDDIT_ACCESS_TOKEN", "REDDIT_USER_AGENT", "REDDIT_SUBREDDIT"]);
...
function missingDirectEnv(provider, env = process.env) {
  ...
  if (provider === "youtube") {
    ...
    if (clean(env.REDDIT_API_APPROVED) !== "true") missing.push("REDDIT_API_APPROVED");
...
export function socialGatewayStatus(env = process.env) {
  return PROVIDERS.map((provider) => {
    const webhook = Boolean(webhookUrl(provider, env));
    const direct = directConfigured(provider, env);
    const approved = provider !== "reddit" || clean(env.REDDIT_API_APPROVED) === "true";
