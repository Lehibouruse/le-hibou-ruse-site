const REQUIREMENTS = {
  ...
  meta: {
    ...
    analytics_all: ["pages_read_engagement", "read_insights", "instagram_manage_insights"],
    blocker: "App Meta, Page Facebook reliée au compte Instagram professionnel et permissions/app review applicables. pages_read_engagement + read_insights couvrent le reporting Page ; instagram_manage_insights est requis pour les insights des médias Instagram.",
  },
  tiktok: {
    label: "TikTok",
...
function effectiveRequirement(provider, requirement, env = process.env, app = {}) {
  ...
  const organizationMode = clean(env?.LINKEDIN_ORGANIZATION_URN)
    || configuredScopes.has("w_organization_social")
    || configuredScopes.has("rw_organization_admin");
