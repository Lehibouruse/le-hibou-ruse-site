...
async function requiredJson(response, label) {
  ...
  if (!response.ok) throw apiError(label, response, data);
  return data;
}
...
async function optionalInsightMetric(base, mediaId, token, metric, fetchImpl) {
  ...
  throw apiError(`Instagram insight ${metric}`, response, data);
}
...
export async function fetchInstagramEnhancedMetrics(mediaId, env = process.env, fetchImpl = fetch, scopes = "") {
  ...
  if (!token || !version) throw credentialMissing("Instagram/Meta credential absent");

  const granted = scopeSet(scopes);
  if (granted.size) {
    const required = ["pages_read_engagement", "instagram_manage_insights"];
    const missing = required.filter((scope) => !granted.has(scope));
    if (missing.length) throw needsReauth(`Meta/Instagram scopes analytics absents: ${missing.join(", ")}; reconnexion/approbation OAuth requise`);
  }
