...
import { fetchInstagramEnhancedMetrics } from "./social-instagram-metrics.mjs";
...
async function optionalFacebookInsight(base, id, token, metric, fetchImpl) {
  ...
  if (!data) return { metric, value: null, available: false };
  const value = insightValue(data);
  return { metric, value, available: value !== null };
}

async function firstAvailableFacebookInsight(base, id, token, metrics, fetchImpl) {
  for (const metric of metrics) {
    const result = await optionalFacebookInsight(base, id, token, metric, fetchImpl);
...
export function normalizeFacebookMetrics(postId, post = {}, extras = {}) {
  const reactions = count(post?.reactions?.summary?.total_count ?? post?.likes?.summary?.total_count ?? 0);
  const comments = count(post?.comments?.summary?.total_count ?? 0);
  const shares = count(post?.shares?.count ?? 0);
  const available = Array.isArray(extras.analytics_metrics) ? extras.analytics_metrics.filter(Boolean) : [];
  const apiInteractions = extras.total_interactions;
  ...
  return {
    ...
    reach: count(extras.reach),
    total_interactions: totalInteractions,
    analytics_status: clean(extras.analytics_status) || (available.length ? "active" : "unavailable"),
...
export async function fetchFacebookMetrics(postId, env = process.env, fetchImpl = fetch, scopes = "") {
  ...
  if (scopesSet.size && !scopesSet.has("pages_read_engagement")) {
    throw needsReauth("Meta/Facebook scope pages_read_engagement absent; reconnexion/approbation OAuth requise");
  }
  ...
  // The post counters remain useful even when the Page Insights permission has
  // not yet been granted. In that case we deliberately skip the insights calls
  // instead of converting the whole refresh into a re-auth failure.
  if (scopesSet.size && !scopesSet.has("read_insights")) {
    return normalizeFacebookMetrics(id, post, { analytics_status: "scope_missing", analytics_metrics: [] });
  }

  const [viewsResult, clicksResult, reachResult] = await Promise.all([
    firstAvailableFacebookInsight(base, id, token, ["post_media_view", "post_video_views", "post_impressions"], fetchImpl),
    firstAvailableFacebookInsight(base, id, token, ["post_clicks", "post_clicks_by_type"], fetchImpl),
    firstAvailableFacebookInsight(base, id, token, ["post_impressions_unique"], fetchImpl),
  ]);
  const results = [viewsResult, clicksResult, reachResult];
  const analyticsMetrics = [...new Set(results.filter((item) => item.available).map((item) => item.metric).filter(Boolean))];
  return normalizeFacebookMetrics(id, post, {
    views: viewsResult.value,
    clicks: clicksResult.value,
    reach: reachResult.value,
    analytics_status: analyticsMetrics.length ? "active" : "unavailable",
...
export async function fetchSocialMetrics(providerInput, externalId, baseEnv = process.env, fetchImpl = fetch) {
  ...
  if (provider === "instagram") {
    const resolved = await resolveSocialEnv("instagram", baseEnv);
    return { ...(await fetchInstagramEnhancedMetrics(externalId, resolved.env, fetchImpl, resolved.scopes || "")), credential_source: resolved.source };
  }
  if (provider === "youtube") {
