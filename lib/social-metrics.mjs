...
export function normalizeYoutubeMetrics(item = {}) {
  ...
  return {
    provider: "youtube", external_id: clean(item.id),
    views: count(stats.viewCount), likes: count(stats.likeCount), comments: count(stats.commentCount),
    shares: 0, saves: 0, watch_time_seconds: 0, completion: 0, clicks: 0, followers_generated: 0,
  };
}

export function normalizeTikTokMetrics(item = {}) {
  return {
    provider: "tiktok", external_id: clean(item.id),
    views: count(item.view_count), likes: count(item.like_count), comments: count(item.comment_count),
    shares: count(item.share_count), saves: 0, watch_time_seconds: 0, completion: 0, clicks: 0, followers_generated: 0,
...
export function normalizeXMetrics(post = {}, media = {}) {
  ...
  const playback100 = count(mediaPrivate.playback_100_count);
  return {
    provider: "x", external_id: clean(post.id),
    views: count(mediaPublic.view_count || publicMetrics.impression_count),
    likes: count(publicMetrics.like_count), comments: count(publicMetrics.reply_count),
    shares: count(publicMetrics.retweet_count) + count(publicMetrics.quote_count),
    saves: count(publicMetrics.bookmark_count), watch_time_seconds: 0,
    completion: ratio(playback100, playback0), clicks: count(privateMetrics.url_link_clicks), followers_generated: 0,
    url: post.id ? `https://x.com/i/web/status/${post.id}` : "",
  };
}
...
export function normalizeThreadsMetrics(threadId, insights = [], thread = {}) {
  ...
  return {
    provider: "threads", external_id: clean(threadId),
    views: count(m.get("views")), likes: count(m.get("likes")), comments: count(m.get("replies")),
    shares: count(m.get("reposts")) + count(m.get("quotes")) + count(m.get("shares")),
    saves: 0, watch_time_seconds: 0, completion: 0, clicks: 0, followers_generated: 0,
    url: clean(thread.permalink),
  };
}
...
export function normalizeInstagramMetrics(mediaId, insights = [], media = {}) {
  const m = insightMap(insights);
  return {
    provider: "instagram", external_id: clean(mediaId),
    views: count(m.get("views") || m.get("plays") || m.get("reach")),
    likes: count(media.like_count), comments: count(media.comments_count), shares: count(m.get("shares")), saves: count(m.get("saved")),
    watch_time_seconds: 0, completion: 0, clicks: 0, followers_generated: 0,
...
export function normalizeLinkedInMetrics(postUrn, values = {}, url = "") {
  return {
    provider: "linkedin", external_id: clean(postUrn),
    views: count(values.IMPRESSION), likes: count(values.REACTION), comments: count(values.COMMENT),
    shares: count(values.RESHARE), saves: count(values.POST_SAVE), watch_time_seconds: count(values.VIDEO_WATCH_TIME),
    completion: 0, clicks: count(values.LINK_CLICKS), followers_generated: count(values.FOLLOWER_GAINED_FROM_CONTENT),
    url: clean(url),
  };
}
...
export async function fetchYoutubeMetrics(videoId, env = process.env, fetchImpl = fetch) {
  ...
  if (!token) throw credentialError("YouTube");
  const url = new URL("https://www.googleapis.com/youtube/v3/videos");
  url.searchParams.set("part", "statistics"); url.searchParams.set("id", id);
...
export async function fetchTikTokMetrics(videoId, env = process.env, fetchImpl = fetch, scopes = "") {
  ...
  if (!token) throw credentialError("TikTok");
  requireScope(scopes, "video.list", "TikTok");
  ...
  const data = await readJson(await fetchImpl(url, {
    ...
    body: JSON.stringify({ filters: { video_ids: [id] } }), cache: "no-store",
  }), "TikTok metrics");
...
export async function fetchXMetrics(postId, env = process.env, fetchImpl = fetch) {
  ...
  if (!id) throw new Error("X external_id absent");
  if (!token) throw credentialError("X");
  const buildUrl = (withPrivate) => {
    const url = new URL(`https://api.x.com/2/tweets/${encodeURIComponent(id)}`);
    url.searchParams.set("tweet.fields", `attachments,public_metrics${withPrivate ? ",non_public_metrics" : ""}`);
    url.searchParams.set("expansions", "attachments.media_keys");
    url.searchParams.set("media.fields", `public_metrics${withPrivate ? ",non_public_metrics" : ""}`);
    return url;
  };
  ...
  const data = await readJson(response, "X metrics");
  if (!data?.data) throw new Error(`X post introuvable: ${id}`);
...
export async function fetchThreadsMetrics(threadId, env = process.env, fetchImpl = fetch, scopes = "") {
  ...
  if (!id) throw new Error("Threads external_id absent");
  if (!token) throw credentialError("Threads");
  requireScope(scopes, "threads_manage_insights", "Threads");
  const insightsUrl = new URL(`https://graph.threads.net/${encodeURIComponent(id)}/insights`);
  insightsUrl.searchParams.set("metric", "views,likes,replies,reposts,quotes,shares");
  insightsUrl.searchParams.set("access_token", token);
  const threadUrl = new URL(`https://graph.threads.net/${encodeURIComponent(id)}`);
  threadUrl.searchParams.set("fields", "id,permalink"); threadUrl.searchParams.set("access_token", token);
  const [insights, thread] = await Promise.all([
    readJson(await fetchImpl(insightsUrl, { cache: "no-store" }), "Threads insights"),
    readJson(await fetchImpl(threadUrl, { cache: "no-store" }), "Threads post"),
  ]);
...
export async function fetchInstagramMetrics(mediaId, env = process.env, fetchImpl = fetch, scopes = "") {
  ...
  if (!token || !version) throw credentialError("Instagram/Meta");
  requireScope(scopes, "pages_read_engagement", "Meta/Instagram");
  const base = `https://graph.facebook.com/${version}`;
  const insightUrl = new URL(`${base}/${encodeURIComponent(id)}/insights`);
  insightUrl.searchParams.set("metric", "views,reach,saved,shares"); insightUrl.searchParams.set("access_token", token);
  const mediaUrl = new URL(`${base}/${encodeURIComponent(id)}`);
  mediaUrl.searchParams.set("fields", "id,permalink,like_count,comments_count"); mediaUrl.searchParams.set("access_token", token);
  const [insights, media] = await Promise.all([
    readJson(await fetchImpl(insightUrl, { cache: "no-store" }), "Instagram insights"),
    readJson(await fetchImpl(mediaUrl, { cache: "no-store" }), "Instagram media"),
  ]);
  return normalizeInstagramMetrics(id, insights?.data || [], media || {});
}

function linkedInEntity(urn) {
  const value = clean(urn);
  if (value.includes(":ugcPost:")) return `(ugc:${encodeURIComponent(value)})`;
...
async function linkedInMetric(postUrn, metric, env, fetchImpl) {
  ...
  if (!version || !token) throw credentialError("LinkedIn");
...
export async function fetchLinkedInMetrics(postUrn, env = process.env, fetchImpl = fetch, scopes = "") {
  ...
  if (!urn) throw new Error("LinkedIn external_id absent");
  if (!clean(env.LINKEDIN_ACCESS_TOKEN)) throw credentialError("LinkedIn");
  requireScope(scopes, "r_member_postAnalytics", "LinkedIn");
  ...
  return normalizeLinkedInMetrics(urn, values, id ? `https://www.linkedin.com/feed/update/${encodeURIComponent(urn)}` : "");
}
...
export async function fetchSocialMetrics(providerInput, externalId, baseEnv = process.env, fetchImpl = fetch) {
  ...
  if (!["youtube", "tiktok", "x", "threads", "instagram", "linkedin"].includes(provider)) {
...
export function contentMetricTargets(record = {}) {
  ...
  const defs = [
    ["youtube", "ID YouTube", "URL YouTube"], ["tiktok", "ID TikTok", "URL TikTok"],
    ["instagram", "ID Instagram", "URL Instagram"], ["linkedin", "ID LinkedIn", "URL LinkedIn"],
    ["x", "ID X", "URL X"], ["threads", "ID Threads", "URL Threads"],
  ];
  return defs.flatMap(([provider, idField, urlField]) => {
    const externalId = clean(fields[idField]);
    return externalId ? [{ provider, external_id: externalId, url: clean(fields[urlField]), content_record_id: record.id }] : [];
  });
}
