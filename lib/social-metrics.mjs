...
export async function fetchInstagramMetrics(mediaId, env = process.env, fetchImpl = fetch, scopes = "") {
  const id = clean(mediaId); const token = clean(env.FACEBOOK_PAGE_ACCESS_TOKEN || env.META_ACCESS_TOKEN); const version = clean(env.META_GRAPH_VERSION);
  if (!id) throw new Error("Instagram external_id absent"); if (!token || !version) throw credentialError("Instagram/Meta"); requireScope(scopes, "pages_read_engagement", "Meta/Instagram");
...
export async function fetchSocialMetrics(providerInput, externalId, baseEnv = process.env, fetchImpl = fetch) {
  ...
  if (!["youtube", "tiktok", "x", "threads", "instagram", "linkedin"].includes(provider)) { const error = new Error(`Métriques ${provider || "provider vide"} pas encore implémentées`); error.code = "unsupported_provider"; throw error; }
...
export function contentMetricTargets(record = {}) {
  ...
  const defs = [["youtube", "ID YouTube", "URL YouTube"], ["tiktok", "ID TikTok", "URL TikTok"], ["instagram", "ID Instagram", "URL Instagram"], ["linkedin", "ID LinkedIn", "URL LinkedIn"], ["x", "ID X", "URL X"], ["threads", "ID Threads", "URL Threads"]];
