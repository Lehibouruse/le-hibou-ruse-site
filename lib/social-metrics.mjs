...
export async function fetchSocialMetrics(providerInput, externalId, baseEnv = process.env, fetchImpl = fetch) {
  ...
  if (!["youtube", "tiktok", "x", "threads", "instagram", "linkedin", "pinterest", "bluesky"].includes(provider)) { const error = new Error(`Métriques ${provider || "provider vide"} pas encore implémentées`); error.code = "unsupported_provider"; throw error; }
...
export function contentMetricTargets(record = {}) {
  ...
  const defs = [["bluesky", "ID Bluesky", "URL Bluesky"], ["youtube", "ID YouTube", "URL YouTube"], ["tiktok", "ID TikTok", "URL TikTok"], ["instagram", "ID Instagram", "URL Instagram"], ["linkedin", "ID LinkedIn", "URL LinkedIn"], ["pinterest", "ID Pinterest", "URL Pinterest"], ["x", "ID X", "URL X"], ["threads", "ID Threads", "URL Threads"]];
