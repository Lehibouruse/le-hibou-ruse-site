import { fetchRedditMetrics } from "./social-reddit.mjs";
import { fetchBlueskyMetrics } from "./social-bluesky-metrics.mjs";
...
export function normalizeLinkedInMetrics(postUrn, values = {}, url = "") {
  return {
    ...
    views: count(values.impressionCount ?? values.IMPRESSION),
    likes: count(values.likeCount ?? values.REACTION),
    comments: count(values.commentCount ?? values.COMMENT),
    shares: count(values.shareCount ?? values.RESHARE),
    saves: null, watch_time_seconds: null, completion: null,
    clicks: count(values.clickCount ?? values.LINK_CLICKS),
    followers_generated: null, url: clean(url),
  };
}
...
function linkedInOrganizationStatisticsUrl(postUrn, organizationUrn) {
  const urn = clean(postUrn);
  const key = urn.includes(":ugcPost:") ? "ugcPosts" : "shares";
  return `https://api.linkedin.com/rest/organizationalEntityShareStatistics?q=organizationalEntity&organizationalEntity=${encodeURIComponent(organizationUrn)}&${key}=List(${encodeURIComponent(urn)})`;
}
export async function fetchLinkedInMetrics(postUrn, env = process.env, fetchImpl = fetch, scopes = "") {
  const urn = clean(postUrn); const token = clean(env.LINKEDIN_ACCESS_TOKEN); const version = clean(env.LINKEDIN_VERSION); const organizationUrn = clean(env.LINKEDIN_ORGANIZATION_URN);
  if (!urn) throw new Error("LinkedIn external_id absent");
  if (!token || !version || !organizationUrn) throw credentialError("LinkedIn");
  requireScope(scopes, "rw_organization_admin", "LinkedIn");
  const data = await readJson(await fetchImpl(linkedInOrganizationStatisticsUrl(urn, organizationUrn), {
  ...
  const values = data?.elements?.[0]?.totalShareStatistics || {};
...
export async function fetchSocialMetrics(providerInput, externalId, baseEnv = process.env, fetchImpl = fetch) {
  ...
  if (!["youtube", "tiktok", "x", "threads", "instagram", "linkedin", "pinterest", "reddit", "bluesky"].includes(provider)) { const error = new Error(`Métriques ${provider || "provider vide"} pas encore implémentées`); error.code = "unsupported_provider"; throw error; }
  const resolved = await resolveSocialEnv(provider, baseEnv);
  if (provider === "reddit") return { ...(await fetchRedditMetrics(externalId, resolved.env, fetchImpl)), credential_source: resolved.source };
  if (provider === "bluesky") return { ...(await fetchBlueskyMetrics(externalId, resolved.env, fetchImpl)), credential_source: "public_api" };
...
export function contentMetricTargets(record = {}) {
  ...
  const defs = [["reddit", "ID Reddit", "URL Reddit"], ["bluesky", "ID Bluesky", "URL Bluesky"], ["youtube", "ID YouTube", "URL YouTube"], ["tiktok", "ID TikTok", "URL TikTok"], ["instagram", "ID Instagram", "URL Instagram"], ["linkedin", "ID LinkedIn", "URL LinkedIn"], ["pinterest", "ID Pinterest", "URL Pinterest"], ["x", "ID X", "URL X"], ["threads", "ID Threads", "URL Threads"]];
