...
export async function fetchSocialMetrics(providerInput, externalId, baseEnv = process.env, fetchImpl = fetch) {
  ...
  if (!["youtube", "tiktok", "x", "threads", "instagram", "linkedin", "pinterest", "bluesky", "reddit"].includes(provider)) { const error = new Error(`Métriques ${provider || "provider vide"} pas encore implémentées`); error.code = "unsupported_provider"; throw error; }
  if (provider === "bluesky") return { ...(await fetchBlueskyMetrics(externalId, baseEnv, fetchImpl)), credential_source: "public_api" };
