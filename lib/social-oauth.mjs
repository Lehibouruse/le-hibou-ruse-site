...
export async function completeSocialAuthorization(providerInput, { code, state: sealedState }, env = process.env) {
  ...
  else throw new Error(`OAuth exchange absent: ${provider}`);
  ...
  return { provider, scopes: result.scopes, expires_at: result.expiresAt, connected: true };
}

export { SUPPORTED as SOCIAL_OAUTH_PROVIDERS, redirectUri as socialOAuthRedirectUri };
