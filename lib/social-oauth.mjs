...
import { requireTikTokUserToken } from "./social-tiktok-token.mjs";
import { redditOAuthConfig, redditToken, redditIdentity } from "./social-reddit.mjs";

const SUPPORTED = ["youtube", "tiktok", "meta", "instagram", "linkedin", "pinterest", "x", "threads", "reddit"];
...
function redirectUri(provider, env = process.env) {
  const base = `${publicBase(env)}/api/social/oauth/${provider}/callback`;
  return provider === "threads" ? `${base}/` : base;
}
...
function config(providerInput, env = process.env) {
  ...
  if (provider === "reddit") return redditOAuthConfig(env);
  ...
  if (provider === "linkedin") return {
    ...
    // Le Hibou publie au nom de la Page/organisation, jamais via w_member_social.
    // rw_organization_admin est demandé pour les statistiques organiques de Page.
    scopes: clean(env.LINKEDIN_OAUTH_SCOPES) || "w_organization_social r_organization_social rw_organization_admin",
  };
  ...
  if (provider === "instagram") {
    ...
    return {
      provider,
      clientId: required(env.INSTAGRAM_APP_ID, "INSTAGRAM_APP_ID"),
      clientSecret: required(env.INSTAGRAM_APP_SECRET, "INSTAGRAM_APP_SECRET"),
      authorizeUrl: clean(env.INSTAGRAM_AUTHORIZE_URL) || "https://www.instagram.com/oauth/authorize",
      tokenUrl: "https://api.instagram.com/oauth/access_token",
      graphBase: `https://graph.instagram.com/${version}`,
      scopes: clean(env.INSTAGRAM_OAUTH_SCOPES) || "instagram_business_basic,instagram_business_content_publish,instagram_business_manage_insights",
    };
  }
  if (provider === "meta") {
    const version = clean(env.META_GRAPH_VERSION) || "v26.0";
    return {
      provider,
      ...
      scopes: clean(env.META_OAUTH_SCOPES) || "pages_show_list,pages_manage_posts,pages_read_engagement,read_insights",
    };
  }
...
export function buildSocialAuthorization(providerInput, env = process.env) {
  ...
  if (cfg.provider === "reddit") params.set("duration", "permanent");
...
async function exchangeTikTok(cfg, code, state, env) {
  const token = requireTikTokUserToken(await formPost(cfg.tokenUrl, {
...
async function exchangeLinkedIn(cfg, code, state, env) {
  ...
  return {
    payload: { env: { LINKEDIN_ACCESS_TOKEN: token.access_token, LINKEDIN_REFRESH_TOKEN: token.refresh_token || "" } },
    scopes: token.scope || cfg.scopes,
    expiresAt: nowIsoPlus(token.expires_in),
  };
}
...
async function exchangeInstagram(cfg, code, state, env) {
  ...
  if (!short.access_token) throw new Error("Instagram OAuth: access_token absent");
  ...
  if (!longResponse.ok || !long.access_token) {
    throw new Error(`Instagram long-lived token ${longResponse.status}: ${long.error?.message || long.error || longResponse.statusText}`);
  }
  const token = long.access_token;
  const profile = await jsonGet(`${cfg.graphBase}/me?fields=id,username,account_type,profile_picture_url`, token).catch(() => ({}));
  const userId = clean(profile.id || short.user_id);
  if (!userId) throw new Error("Instagram OAuth: identifiant professionnel absent");
  return {
    payload: {
      env: {
        INSTAGRAM_ACCESS_TOKEN: token,
        INSTAGRAM_BUSINESS_ACCOUNT_ID: userId,
        INSTAGRAM_USERNAME: clean(profile.username),
      },
      profile,
    },
    scopes: short.permissions || cfg.scopes,
...
async function exchangeReddit(cfg, code, state, env) {
  const token = await redditToken({ grant_type: "authorization_code", code, redirect_uri: redirectUri(cfg.provider, env) }, env);
  if (!clean(token.refresh_token)) throw new Error("Reddit : refresh token absent ; autorisation permanente requise");
  const profile = await redditIdentity({ ...env, REDDIT_ACCESS_TOKEN: token.access_token });
  return {
    payload: { env: { REDDIT_ACCESS_TOKEN: token.access_token, REDDIT_REFRESH_TOKEN: token.refresh_token, REDDIT_USERNAME: profile.publicId }, profile },
    scopes: token.scope || cfg.scopes,
    expiresAt: nowIsoPlus(token.expires_in),
  };
}
...
export async function completeSocialAuthorization(providerInput, { code, state: sealedState }, env = process.env) {
  ...
  const cfg = config(provider, env);
  let result;
  if (provider === "reddit") result = await exchangeReddit(cfg, code, state, env);
  else if (provider === "youtube") result = await exchangeYoutube(cfg, code, state, env);
  ...
  else if (provider === "instagram") result = await exchangeInstagram(cfg, code, state, env);
