...
const DEFAULT_PUBLIC_BASE_URL = "https://le-hibou-ruse-site.vercel.app";
...
function publicBase(env = process.env) {
  const candidate = clean(env.HIBOU_PUBLIC_BASE_URL) || DEFAULT_PUBLIC_BASE_URL;
...
function config(providerInput, env = process.env) {
  ...
  if (provider === "tiktok") return {
    ...
    scopes: clean(env.TIKTOK_OAUTH_SCOPES) || "user.info.basic,video.publish,video.upload",
  };
  ...
  if (provider === "meta") {
    ...
    return {
      ...
      scopes: clean(env.META_OAUTH_SCOPES) || "pages_show_list,pages_manage_posts,instagram_basic,instagram_content_publish",
    };
  }
  if (provider === "threads") return {
    ...
    scopes: clean(env.THREADS_OAUTH_SCOPES) || "threads_basic,threads_content_publish",
  };
...
export async function completeSocialAuthorization(providerInput, { code, state: sealedState }, env = process.env) {
  ...
  if (state.provider !== cfg.provider) throw new Error("OAuth provider/state incohérent");
  const authCode = required(code, "code OAuth");
  let exchanged;
  if (cfg.provider === "youtube") exchanged = await exchangeYoutube(cfg, authCode, state, env);
  else if (cfg.provider === "tiktok") exchanged = await exchangeTikTok(cfg, authCode, state, env);
  else if (cfg.provider === "linkedin") exchanged = await exchangeLinkedIn(cfg, authCode, state, env);
  else if (cfg.provider === "x") exchanged = await exchangeX(cfg, authCode, state, env);
  else if (cfg.provider === "meta") exchanged = await exchangeMeta(cfg, authCode, state, env);
  else if (cfg.provider === "threads") exchanged = await exchangeThreads(cfg, authCode, state, env);
  else throw new Error("Provider OAuth non géré");

  const record = await storeSocialCredential({
  ...
  return { provider: cfg.provider, record_id: record?.records?.[0]?.id || "", scopes: exchanged.scopes, expires_at: exchanged.expiresAt };
}
