export const SOCIAL_PROVIDERS = ["youtube", "instagram", "facebook", "threads", "tiktok", "linkedin", "pinterest", "x", "snapchat"];
...
export function socialRuntimeEnv(config = {}, baseEnv = process.env) {
  ...
  for (const provider of SOCIAL_PROVIDERS) {
    ...
    if (["auto", "direct", "webhook"].includes(requested)) env[`HIBOU_SOCIAL_${provider.toUpperCase()}_MODE`] = requested;
  }
  const scopes = {
    youtube: "YOUTUBE_OAUTH_SCOPES", meta: "META_OAUTH_SCOPES", tiktok: "TIKTOK_OAUTH_SCOPES",
    linkedin: "LINKEDIN_OAUTH_SCOPES", threads: "THREADS_OAUTH_SCOPES", x: "X_OAUTH_SCOPES", pinterest: "PINTEREST_OAUTH_SCOPES",
  };
  for (const [provider, envName] of Object.entries(scopes)) {
    const value = String(config[`social_oauth_scopes_${provider}`] || "").trim();
