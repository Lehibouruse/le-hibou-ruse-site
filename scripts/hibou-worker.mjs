...
async function openPullRequest(pushed, job) {
  ...
  const response = await fetch("https://api.github.com/repos/Lehibouruse/le-hibou-ruse-site/pulls", {
    ...
    body: JSON.stringify({ title: `Hibou agent: ${job.fields.job_id}`, head: branch, base: "main", body: `Proposition autonome bornée pour le job ${job.fields.job_id}. Tests et build exécutés. Aucune fusion automatique.` }),
  });
  const data = await response.json().catch(() => ({}));
  ...
  return { branch, commit, changed_paths: changedPaths, pull_request: data.html_url, merged: merge.ok && mergeData.merged === true, merge_message: mergeData.message || "" };
}
