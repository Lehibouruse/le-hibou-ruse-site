...
function firstAvailable(candidates) {
  ...
  return { available: false, status: null, command: candidates.map(x => x.join(" ")).join(" | "), stdout: "", stderr: "" };
}
...
export function collectPreflight(root = process.cwd()) {
  ...
  const python311 = process.env.HIBOU_PYTHON
    ? command(process.env.HIBOU_PYTHON, ["--version"])
    : firstAvailable(process.platform === "win32"
      ? [["py", "-3.11", "--version"], ["python3.11", "--version"]]
      : [["python3.11", "--version"]]);
  const pythonDefault = firstAvailable(process.platform === "win32"
  ...
  const report = {
    ...
    python_3_11: { available: python311.available, command: python311.command, version: python311.stdout || python311.stderr },
    python_default: { available: pythonDefault.available, command: pythonDefault.command, version: pythonDefault.stdout || pythonDefault.stderr },
