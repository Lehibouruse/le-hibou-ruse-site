...
test("le webhook supporte le lecteur Hibou sans dépendre de Digify", () => {
  const lemon = readFileSync(new URL("../app/api/commerce/lemon-webhook/route.js", import.meta.url), "utf8");
  assert.match(lemon, /"digify", "lemon_native", "hibou_reader"/);
  assert.match(lemon, /hibouReaderProviderReady/);
  assert.match(lemon, /"reader_ready"/);
  assert.match(lemon, /lemon_downloadable_file_removed_verified/);
});
