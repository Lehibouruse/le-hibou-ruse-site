...
test("TikTok AUTHORIZED conserve seulement la contrainte d'audit public tant qu'elle existe", () => {
  const env = {
  ...
  const credentials = [{ provider: "tiktok", status: "Connected", scopes: "user.info.basic video.publish video.list" }];
  const snapshot = buildSocialControlPlane({ readiness, credentials, env });
  const tiktok = snapshot.providers.find((item) => item.provider === "tiktok");
