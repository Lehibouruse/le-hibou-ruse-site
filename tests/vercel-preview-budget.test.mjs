...
test("Vercel reste désactivé par défaut et ne déploie que main", () => {
...
test("les changements backend lib déclenchent bien un déploiement main", () => {
  assert.match(vercel.ignoreCommand, /-- app components lib public/);
});

test("le worker conserve son garde-fou historique de branche même si les previews agent sont désormais désactivées", () => {
